...
class AnotacaoHelper {
  ...
  get db async {
    //verifica se ja existe uma instancia do banco de dados
  ...
  inicializarDB() async {
    ...
    var db = await openDatabase(localBancoDados,
        version: 1,
        onCreate: _onCreate); //chama funcao para criacao do banco de dados
    return db;
  }

  Future<int> salvarAnotacao(Anotacao anotacao) async {
    //chama a classse anotacao
  ...
  recuperarAnotacoes() async {
    var bancoDados = await db; //configura banco de dados
    String sql =
        "SELECT * FROM $nomeTabela ORDER BY data DESC "; //sql de consulta no banco de dado
